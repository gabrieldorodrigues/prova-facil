...
export function ExamDetailScreen({ route, navigation }: Props) {
  ...
  const handleExport = async () => {
    if (corrections.length === 0) {
      Alert.alert(
        "Sem correções",
        "Corrija pelo menos um aluno antes de exportar.",
      );
      return;
    }
    try {
  ...
  const handleDelete = () => {
    Alert.alert(
      "Excluir avaliação?",
      "Todas as correções desta avaliação serão removidas.",
      [
        ...
        <Card.Content>
          {exam.questions.map((q) => (
            <View key={q.id} style={styles.gabaritoRow}>
              <Text style={[styles.qNumber, { color: theme.colors.onSurface }]}>
                {q.number}.
              </Text>
              <Text
                style={[styles.qType, { color: theme.colors.onSurfaceVariant }]}
              >
